...
func writePID(at path: String) {
    do {
        try "\(ProcessInfo.processInfo.processIdentifier)".write(
            toFile: path, atomically: true, encoding: .utf8)
    } catch {
        log("Warning: Could not write PID file \(path): \(error.localizedDescription)")
    }
}

func cleanupPID(at path: String) {
    unlink(path)
}

func killStalePID(at path: String) {
...
public class MicKeeper: NSObject, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    public init(pidPath: String = "/tmp/mic-warm.pid") {
        self.pidPath = pidPath
    }

    public func start() {
        killStalePID(at: pidPath)
        writePID(at: pidPath)
